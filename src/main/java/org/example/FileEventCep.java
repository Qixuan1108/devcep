...
public class FileEventCep {
    public static void main(String[] args) throws Exception {
        BufferedWriter out = new BufferedWriter(new FileWriter("/root/env1/file-result.txt"));
        int i = 0;
        while(i<100) {
            ...
            Pattern<Event, Event> MyPattern = Pattern.<Event>begin("start").where(new IterativeCondition<Event>() {
                ...
                public boolean filter(Event value, Context<Event> ctx) throws Exception {
                    if (value.getAttribute("eventType").isPresent() && value.getAttribute("eventType").get().equals("C")) {
                        List<Event> contextEventsA = Lists.newArrayList(ctx.getEventsForPattern("start"));
                        List<Event> contextEventsB = Lists.newArrayList(ctx.getEventsForPattern("pre-middle"));
                        if (contextEventsA.size() > 0 && contextEventsB.size() > 0) {
                            Event first_A = contextEventsA.get(0);
                            Event first_B = contextEventsB.get(0);
                            long interval_AC = (long) value.getAttribute("startTimeStamp").get() -
                                    (long) first_A.getAttribute("startTimeStamp").get();
                            long interval_BC = (long) value.getAttribute("startTimeStamp").get() -
                                    (long) first_B.getAttribute("startTimeStamp").get();
                            if (interval_AC < 9000 && interval_BC > 2000 && interval_BC < 5000) {
                ...
                public boolean filter(Event value, Context<Event> ctx) throws Exception {
                    if (value.getAttribute("eventType").isPresent() && value.getAttribute("eventType").get().equals("D")) {
                        List<Event> contextEventsA = Lists.newArrayList(ctx.getEventsForPattern("start"));
                        List<Event> contextEventsB = Lists.newArrayList(ctx.getEventsForPattern("pre-middle"));
                        List<Event> contextEventsC = Lists.newArrayList(ctx.getEventsForPattern("suf-middle"));
                        if (contextEventsA.size() > 0 && contextEventsB.size() > 0 && contextEventsC.size() > 0) {
                            Event first_A = contextEventsA.get(0);
                            Event first_B = contextEventsB.get(0);
                            Event first_C = contextEventsC.get(0);
                            long interval_AD = (long) value.getAttribute("startTimeStamp").get() -
                                    (long) first_A.getAttribute("startTimeStamp").get();
                            long interval_BD = (long) value.getAttribute("startTimeStamp").get() -
                                    (long) first_B.getAttribute("startTimeStamp").get();
                            long interval_CD = (long) value.getAttribute("startTimeStamp").get() -
                                    (long) first_C.getAttribute("startTimeStamp").get();
                            if (interval_BD < 8000 && interval_BD > 4000 && interval_CD > 1000 && interval_CD < 7000 &&
                                    interval_AD < 17000 && interval_AD > 6000) {
